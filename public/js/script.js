...
if (window.location.pathname.endsWith('results.html')) {
    ...
    if (results && results.length > 0) {
        ...
        results.forEach(result => {
            ...
            editButton.setAttribute('data-id', result.id);
            const deleteButton = document.createElement('button');
            deleteButton.textContent = 'Eliminar';
            deleteButton.className = 'buttons delete';
            deleteButton.setAttribute('data-id', result.id);
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            // Obtiene el registro según el ID.
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            // Emite una solicitud fetch (DELETE) a la API, con el ID como parámetro.
...
if (window.location.pathname.endsWith('editor.html')) {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (playerData) {
            ...
            document.getElementById('edit-form').dataset.id = player.id; // Almacena el ID en el formulario, como información adicional.
        } else {
    ...
    editFormElement.addEventListener('submit', (event) => {
        ...
        const item = {
            ...
            valormercado: document.getElementById('marketvalue').value,
            imagen: document.getElementById('image').value,
            id: parseInt(editFormElement.dataset.id)
        };
